...
getUser().then((d) => console.log("Problem 21: " + d.name));
// ==============================================
...
async function loadDashboard() {
  ...
  const data = await Promise.all([getProducts(), getOrders(), getUser2()]);
  return data;
}
...
runAll().then((d) => {
  d.forEach((a) => {
    ...
    if (a.status === "fulfilled") {
      console.log(`fulfilled:${a.value}`);
    } else {
      console.log(`rejected:${a.reason.message}`);
    }
  });
});
// Expected:
...
const mockFetchUser = (id) =>
  new Promise((res) =>
    setTimeout(() => res({ name: "Sabbir", balance: 500 }), 200),
  );
const mockFetchProduct = (id) =>
  new Promise((res) =>
    setTimeout(() => res({ name: "Laptop", price: 300 }), 200),
  );

async function checkout(userId, productId) {
  // Your code here:
  try {
    const transection = await Promise.all([
      mockFetchProduct(productId),
      mockFetchUser(userId),
    ]);
    console.log("Problem 24:");
    const [{ price }, { balance }] = transection;
    if (balance >= price) console.log("Purchase successful");
